...
IWTimage <- function(
  ...
  if (inherits(IWT_result, "IWT1") || inherits(IWT_result, "IWT2")) {
    ...
    for (i in 0:(p - 1)) {
      for (j in 1:(2 * p)) {
        heatmap_matrix[p - i, j + i + p] <- IWT_result$pval_matrix[
          p - i,
          (j + 1) %/% 2
        ]
        if (j + i > 2 * p - i) {
    ...
    graphics::image(
      1,
      seq(0, 1, length.out = nlevel) -
        0.025 * seq(0, 1, length.out = nlevel) +
    ...
    for (i in 0:(p - 1)) {
      for (j in 1:(2 * p)) {
        heatmap_matrix_F[p - i, j + i + p] <- IWT_result$pval_matrix_F[
          p - i,
          (j + 1) %/% 2
        ]
        if (j + i > 2 * p - i) {
