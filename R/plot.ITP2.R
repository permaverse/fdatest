...
#'
#' \code{plot} method for class "\code{ITP2}". Plotting function creating a
...
#' 
#' @param x The object to be plotted. An object of class "\code{ITP2}", that is,
#'   a result of an ITP for comparison between two populations. Usually a call
#'   to \code{\link{ITP2bspline}}, \code{\link{ITP2fourier}} or
...
#'   significant differences. Default is \code{alpha1 = 0.05}.
#' @param alpha2 Second level of significance used to select and display
#'   significant differences. Default is \code{alpha1 = 0.01}. \code{alpha1} and
...
#' @param main An overall title for the plots (it will be pasted to
#'   "\code{Functional Data}" for the first plot and "\code{adjusted p-values}"
#'   for the second plot).
...
#' @param \dots Additional plotting arguments that can be used with function
...
#'   \code{alpha1} and \code{alpha2} are highlighted in the plot of the adjusted
...
#'   one for amplitude.
#' 
#' @seealso \code{\link{ITPimage}} for the plot of p-values heatmaps. See also
#'   \code{\link{ITP2bspline}}, \code{\link{ITP2fourier}},
#'   \code{\link{ITP2pafourier}} to perform the ITP to test for differences
#'   between two populations. See \code{\link{plot.ITP1}} and
#'   \code{\link{plot.ITPlm}} for the plot method applied to the ITP results of
...
plot.ITP2 <- function(x, 
  ...
  if(object$basis=='Fourier'){
    ...
    fda::matplot(Abscissa,t(object$data.eval),type='l',main=main.data,ylab=ylab,col=colors,lwd=lwd,ylim=ylim,...)
    ...
    fda::matplot(Abscissa,t(object$data.eval),type='l',main=main.data,ylab=ylab,col=colors,lwd=lwd,add=TRUE,...)
    ...
    graphics::points(1:p,object$adjusted.pval_phase,pch=pch)
    ...
    graphics::points(1:p,object$adjusted.pval_amplitude,pch=pch)
