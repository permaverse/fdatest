...
The `S3` methods `autoplot.faov()` and `plot.faov()` are methods for
...
The `autoplot.faov()` function creates a ggplot object that displays the
...
`plot.faov()` function is a wrapper around `autoplot.faov()` that prints
the plot directly.
